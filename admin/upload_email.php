...
$message = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['html_file']) && $_FILES['html_file']['error'] === UPLOAD_ERR_OK) {
    ...
    if (file_exists($emailListFile)) {
        ...
        if (!empty($emails)) {
            ...
            try {
                ...
                $mail->Body = file_get_contents($file['tmp_name']);

                $mail->send();
                record_sent_email($emails, $subject, file_get_contents($file['tmp_name']));
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
    <h1 class="text-3xl font-bold text-gray-900 mb-8">Upload and Send HTML Email</h1>
    ...
    <form method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            <label for="html_file" class="block text-sm font-medium text-gray-700">HTML File</label>
